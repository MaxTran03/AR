...
void site(int rang){
	...
	if(rang == 1){
		...
		reqs.resp = resp;
		reqs.id = id;
		MPI_Send(&reqs,8, MPI_INT, rangsucc, TAGINIT, MPI_COMM_WORLD);
	}
	
	MPI_Recv(&reqr, 8, MPI_INT, MPI_ANY_SOURCE, TAGINIT, MPI_COMM_WORLD, &status);
		
	if(reqr.req == 0){
		if(rang == reqr.rangsucc){
			resp = reqr.resp;
			MPI_Send(&reqr, 8, MPI_INT, rangsucc, TAGINIT, MPI_COMM_WORLD);
		}else{
			if(reqr.rang == rangsucc){
				succ = reqr.succ;
				rangsucc = reqr.rangsucc;
				reqr.req=5;
				MPI_Send(&reqr, 8, MPI_INT, reqr.rang, TAGINIT, MPI_COMM_WORLD); //confirmation deconnexion
			}else
				MPI_Send(&reqr, 8, MPI_INT, rangsucc, TAGINIT, MPI_COMM_WORLD);
		}
	}else{
	...
	printf("MAJ: id_chord %d, resp %d, succ %d, rangsucc %d\n",id, resp, succ, rangsucc);
	...
	if(rang==2){
		...
		strcpy(data,"La colline a des yeux episode 13");
...
int main (int argc, char* argv[]) {
	...
	p = 1<<M;
	for(i=0;i<p;i++){
		strcpy(film,"La colline a des yeux episode ");
		sprintf(ep, "%d", i);
